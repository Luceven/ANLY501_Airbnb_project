# -*- coding: utf-8 -*-
"""
Created on Sat Oct  6 18:14:52 2018

hotel price web scraping @ Hotels.com
...
def parse(url):
	...
	review_count = re.findall(r'\d+(?:,\d+)?',''.join(raw_review_count).strip())[0].replace(",","") if raw_review_count else None
	...
	if raw_hotel_id:
		...
		hotel_details_response = requests.post(hotel_details_url, headers=hotels_details_request_headers, data=hotel_details_formadata).text
...
if __name__=='__main__':
	...
	#url = args.url
    url = 'https://www.tripadvisor.com/Hotel_Review-g60763-d1776857-Reviews-The_Langham_New_York_Fifth_Avenue-New_York_City_New_York.html'
